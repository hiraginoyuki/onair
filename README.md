...
For streamed responses, `metadata.json` also includes a body-free `stream_usage` summary with the number of observed `usage` objects and the union of their field names. This is intended for checking whether an upstream emitted usage metadata without storing SSE response text, prompt text, completion text, or tool arguments.
