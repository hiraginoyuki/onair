...
For streamed responses, `metadata.json` also includes a body-free `stream_usage` summary with the number of observed `usage` objects, the union of their field names, observed SSE event/type/object names, and the event/type/object names that carried usage metadata. This is intended for checking whether an upstream emitted usage metadata without storing SSE response text, prompt text, completion text, or tool arguments.
