...
- The request table includes sortable columns, identity by default, a column chooser, quick filters for errors/fallback/slow requests, and a filter input that uses space-separated terms so every term must match the retained request metadata it searches.
