...
//! Request parity normalizes system/developer instruction placement and
//! compares portable input/output token usage, rather than provider-generated
//! identifiers or provider-specific cache-token fields.
...
mod tests {
    ...
    use super::*;
    use llm_protocol_core::{ANTHROPIC_MESSAGES_PROFILE, FinishReason, ProtocolResponse, Usage};
    use onair_core::{
        config::{
            ChatStreamUsagePolicy, ResponsesMaxOutputTokensPolicy, ResponsesStorePolicy,
            ToolSchemaMode,
        },
        openai::{
    ...
    fn openai_request_body(value: Value, source: &str, target: &str) -> (Value, Value) {
        let source_profile = profile(source);
        let target_profile = profile(target);
        let wire = openai::WireEnvelope {
            profile_id: source_profile.clone(),
            status: 200,
            body_kind: ProtocolBodyKind::Json,
            protocol_headers: vec![
                llm_protocol_core::ProtocolHeaderLine::new("content-type: application/json")
                    .unwrap(),
            ],
            body: serde_json::to_vec(&value).unwrap(),
            adapter_metadata: AdapterMetadata::default(),
        };
        let decoded = openai::decode(wire.retained_wire(), AdapterMetadata::default())
            .unwrap()
            .output
            .unwrap();
        let alpha = openai::encode_decoded(&decoded, &target_profile)
            .unwrap()
            .output
            .unwrap()
            .wire;
        let onair_mode = match (source, target) {
            (OPENAI_CHAT_COMPLETIONS_PROFILE, OPENAI_RESPONSES_PROFILE) => {
                RequestMode::ChatCompletionsViaResponses
            }
            (OPENAI_RESPONSES_PROFILE, OPENAI_CHAT_COMPLETIONS_PROFILE) => {
                RequestMode::ResponsesViaChatCompletions
            }
            _ => panic!("test has an unsupported OpenAI parity pair"),
        };
        let path = match onair_mode {
            RequestMode::ChatCompletionsViaResponses => "/v1/chat/completions",
            RequestMode::ResponsesViaChatCompletions => "/v1/responses",
            _ => unreachable!(),
        };
        let onair = rewrite_request_body_for_mode_with_policies(
        ...
        .unwrap();
        let alpha_ir = openai::decode(alpha.retained_wire(), AdapterMetadata::default())
            .unwrap()
            .output
            .unwrap()
            .into_value();
        let onair_ir = openai::decode(
            openai::WireEnvelope {
        ...
        .unwrap()
        .into_value();
        (request_semantics(&alpha_ir), request_semantics(&onair_ir))
    }

    fn request_semantics(payload: &ProtocolPayload) -> Value {
        ...
        for message in &request.messages {
            if matches!(
                ...
                instructions.extend(message.content.iter().filter_map(text_content));
            }
        }
        ...
        json!({
            ...
            "cache_intent": request.cache_intent,
        })
    }
    ...
    fn chat_request_to_responses_matches_selected_onair_semantics() {
        ...
        let (alpha, onair) = openai_request_body(
            request,
            OPENAI_CHAT_COMPLETIONS_PROFILE,
            OPENAI_RESPONSES_PROFILE,
        );
        assert_eq!(alpha, onair);
    }
    ...
    fn chat_request_to_messages_matches_selected_onair_semantics() {
        let source_profile = profile(OPENAI_CHAT_COMPLETIONS_PROFILE);
        let target_profile = profile(ANTHROPIC_MESSAGES_PROFILE);
        let request = json!({
            "model": "synthetic-model",
            "messages": [
                {"role": "system", "content": "Synthetic system."},
        ...
        .unwrap();
        let alpha_wire =
            anthropic::encode_canonical(decoded.edit(|_| {}).into_canonical(), &target_profile)
                .unwrap()
                .output
                .unwrap()
                .wire;
        ...
        .unwrap();
        let alpha_ir = anthropic::decode(alpha_wire.retained_wire(), AdapterMetadata::default())
            .unwrap()
            .output
            .unwrap()
            .into_value();
        let onair_ir = anthropic::decode(
            anthropic::WireEnvelope {
                ...
                protocol_headers: vec![
                    llm_protocol_core::ProtocolHeaderLine::new("content-type: application/json")
                        .unwrap(),
                    llm_protocol_core::ProtocolHeaderLine::new("anthropic-version: 2023-06-01")
                        .unwrap(),
                ],
                body: onair,
                adapter_metadata: AdapterMetadata::default(),
        ...
        .unwrap()
        .into_value();
        assert_eq!(request_semantics(&alpha_ir), request_semantics(&onair_ir));
    }

    #[test]
    fn messages_response_to_chat_matches_selected_onair_semantics() {
        let source_profile = profile(ANTHROPIC_MESSAGES_PROFILE);
        let target_profile = profile(OPENAI_CHAT_COMPLETIONS_PROFILE);
        let response = json!({
            "id": "msg_synthetic",
            "type": "message",
            "role": "assistant",
        ...
        .unwrap();
        let alpha_wire =
            openai::encode_canonical(decoded.edit(|_| {}).into_canonical(), &target_profile)
                .unwrap()
                .output
                .unwrap()
                .wire;
        ...
        let alpha = openai::decode(alpha_wire.retained_wire(), AdapterMetadata::default())
            .unwrap()
            .output
            .unwrap()
            .into_value();
        let onair = openai::decode(
            openai::WireEnvelope {
                profile_id: target_profile,
                status: 200,
                body_kind: ProtocolBodyKind::Json,
        ...
        .unwrap()
        .into_value();
        assert_eq!(response_semantics(&alpha), response_semantics(&onair));
    }

    fn response_semantics(payload: &ProtocolPayload) -> Value {
        let ProtocolPayload::Response(ProtocolResponse {
            model,
            output,
            usage,
            finish_reason,
    ...
    #[test]
    fn response_semantics_ignores_provider_generated_identifier() {
        let base = ProtocolPayload::Response(ProtocolResponse {
            ...
            finish_reason: FinishReason::new(FinishReason::STOP).unwrap(),
            continuation: None,
            extensions: Vec::new(),
        });
        let changed_id = ProtocolPayload::Response(ProtocolResponse {
            id: Some("second".to_owned()),
            ..match base.clone() {
                ProtocolPayload::Response(response) => response,
                _ => unreachable!(),
            }
        });
        assert_eq!(response_semantics(&base), response_semantics(&changed_id));
    }
}
